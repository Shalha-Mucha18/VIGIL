...
// === BDT Exchange Rates ===
const YF_RATE_BASE = 'https://query1.finance.yahoo.com/v8/finance/chart';

async function fetchBDTRates() {
  const pairs = { CHF: 'CHFBDT=X', EUR: 'EURBDT=X', USD: 'USDBDT=X', INRUSD: 'INRUSD=X' };
  const results = await Promise.allSettled(
    Object.entries(pairs).map(async ([key, sym]) => {
      try {
        const res = await fetch(`${YF_RATE_BASE}/${sym}?range=1d&interval=1d`, {
  ...
  for (const r of results) {
    if (r.status === 'fulfilled' && r.value[1]) rates[r.value[0]] = r.value[1];
  }
  if (rates.INRUSD && rates.USD) rates.INR = Math.round(rates.INRUSD * rates.USD * 10000) / 10000;
  delete rates.INRUSD;
  return rates;
}

// === Bangladesh Cement News Feed ===

// Known Bangladeshi newspaper domains — used to filter Google News results
const BD_DOMAINS = [
  'thedailystar.net', 'dhakatribune.com', 'prothomalo.com',
  'thefinancialexpress.com.bd', 'newagebd.net', 'daily-sun.com',
  'tbsnews.net', 'bdnews24.com', 'thebusinesspost.net',
  'unb.com.bd', 'bssnews.net', 'theindependentbd.com',
  'bangladeshpost.net', 'observerbd.com', 'risingbd.com',
];

// Fetch Google News RSS and extract <source url=""> tag (actual newspaper domain)
async function fetchGoogleNewsBD(url) {
  try {
    const res = await fetch(url, { signal: AbortSignal.timeout(8000) });
    const xml = await res.text();
    const items = [];
    ...
    while ((match = itemRegex.exec(xml)) !== null) {
      const block = match[1];
      const fullTitle = (block.match(/<title>(?:<!\[CDATA\[)?(.*?)(?:\]\]>)?<\/title>/)?.[1] || '').trim();
      const link = sanitizeExternalUrl((block.match(/<link>(?:<!\[CDATA\[)?(.*?)(?:\]\]>)?<\/link>/)?.[1] || '').trim());
      const pubDate = block.match(/<pubDate>(.*?)<\/pubDate>/)?.[1] || '';
      const sourceUrl = block.match(/<source url="([^"]*)"/)?.[1] || '';
      const sourceName = block.match(/<source[^>]*>([^<]*)<\/source>/)?.[1]?.trim() || '';

      // Only keep articles from Bangladeshi newspapers
      const isBD = BD_DOMAINS.some(d => sourceUrl.includes(d));
      if (!isBD) continue;

      // Strip " - Source Name" suffix Google appends to titles
      const lastDash = fullTitle.lastIndexOf(' - ');
      const headline = lastDash > 0 ? fullTitle.substring(0, lastDash).trim() : fullTitle;

      if (headline) items.push({ title: headline, date: pubDate, source: sourceName || 'BD News', url: link || undefined });
    }
    return items;
  } catch (e) {
    return [];
...
async function fetchBDCementNews() {
  const CEMENT_KEYWORDS = ['cement', 'concrete', 'construction', 'clinker', 'building material', 'infrastructure'];

  const googleUrls = [
    'https://news.google.com/rss/search?q=bangladesh+cement&hl=en-BD&gl=BD&ceid=BD:en',
    'https://news.google.com/rss/search?q=bangladesh+construction+cement&hl=en&gl=US&ceid=US:en',
  ];

  // Direct BD newspaper feeds — filter by cement keywords
  const directFeeds = [
    ['https://en.prothomalo.com/feed', 'Prothom Alo'],
  ];

  const [googleResults, directResults] = await Promise.all([
    Promise.allSettled(googleUrls.map(url => fetchGoogleNewsBD(url))),
    Promise.allSettled(directFeeds.map(([url, src]) => fetchRSS(url, src))),
  ]);
  ...
  for (const r of directResults) {
    ...
    for (const item of r.value) {
      if (CEMENT_KEYWORDS.some(kw => item.title.toLowerCase().includes(kw))) {
        allItems.push(item);
      }
    }
  }
  ...
  for (const item of allItems) {
    ...
    seen.add(key);
    feed.push({
      headline: item.title.substring(0, 120),
      source: item.source,
      timestamp: item.date,
      url: item.url,
      urgent: false,
...
export async function synthesize(data) {
  ...
  // Bangladesh district-level weather (Open-Meteo, no auth)
  const bdwRaw = data.sources['Bangladesh-Weather'] || {};
  const bdWeather = {
    summary: bdwRaw.summary || null,
    signals: (bdwRaw.signals || []).slice(0, 10),
    notable: (bdwRaw.districts || [])
      .filter(d => !d.error && d.weatherCode != null && (d.weatherCode >= 45 || d.temperature >= 38))
      .slice(0, 20)
      .map(d => ({ district: d.district, lat: d.lat, lon: d.lon, condition: d.condition, temperature: d.temperature, precipitation: d.precipitation, weatherCode: d.weatherCode })),
    allDistricts: (bdwRaw.districts || [])
      .filter(d => !d.error)
      .map(d => ({ district: d.district, lat: d.lat, lon: d.lon, temperature: d.temperature, condition: d.condition, weatherCode: d.weatherCode })),
  };
  ...
  const bdRates = await fetchBDTRates();
  const markets = {
    ...
    materials: (yfData.materials || []).map(q => ({
      symbol: q.symbol, name: q.name, price: q.price,
      change: q.change, changePct: q.changePct
    })),
    bd_cement: (yfData.bd_cement || []).map(q => ({
      symbol: q.symbol, name: q.name, price: q.price,
      change: q.change, changePct: q.changePct, currency: q.currency
    })),
    vix: yfQuotes['^VIX'] ? {
  ...
  // Fetch RSS + BD Cement News in parallel
  const [news, bdCementFeed] = await Promise.all([fetchAllNews(), fetchBDCementNews()]);
  ...
  const V2 = {
    ...
    who, fred, energy, bls, treasury, gscpi, defense, noaa, epa, acled, gdelt, space, health, news, bdWeather,
