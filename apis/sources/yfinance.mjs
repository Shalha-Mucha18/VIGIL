...
const SYMBOLS = {
  ...
  EXP: 'Eagle Materials',
  CX: 'CEMEX',
  // Bangladesh region cement (parent companies & South Asia proxies)
  'HOLN.SW': 'Holcim (Lafarge BD parent)',
  'HEI.DE': 'HeidelbergCement (BD parent)',
...
async function fetchQuote(symbol) {
  try {
    ...
    if (data?.error) return { symbol, name: SYMBOLS[symbol] || symbol, error: data.error };
    const result = data?.chart?.result?.[0];
    if (!result) return { symbol, name: SYMBOLS[symbol] || symbol, error: 'no data' };
...
export async function collect() {
  ...
  return {
    ...
    materials: pickGroup(quotes, ['XLB', 'VMC', 'MLM', 'SUM', 'EXP', 'CX']),
    bd_cement: pickGroup(quotes, ['HOLN.SW', 'HEI.DE', 'ULTRACEMCO.NS', 'AMBUJACEM.NS']),
  };
}
