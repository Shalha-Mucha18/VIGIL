...
import { briefing as bangladeshWeather } from './sources/bangladesh-weather.mjs';
...
export async function fullBriefing() {
  ...
  const allPromises = [
    ...
    // Tier 3: Weather, Environment, Technology, Social
    runSource('NOAA', noaa),
    runSource('Bangladesh-Weather', bangladeshWeather),
    runSource('EPA', epa),
...
if (entryHref && import.meta.url === entryHref) {
  ...
  // Save market CSVs (market-data, cement-data, bd-cement-data)
  try {
    const { appendMarketCSV } = await import('../lib/storage/market-csv.mjs');
    const { dirname, join } = await import('path');
    const { fileURLToPath } = await import('url');
    const runsDir = join(dirname(fileURLToPath(import.meta.url)), '../runs');
    const yfData = data.sources?.YFinance;
    if (yfData) {
      await appendMarketCSV(yfData, runsDir);
      console.error('[Crucix] Market CSVs saved to runs/');
    }
  } catch (e) {
    console.error('[Crucix] CSV save failed:', e.message);
  }
